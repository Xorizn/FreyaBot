...
const pino = require("pino"),
  ...
  fs = require("fs"),
  baileys = fs.existsSync("./node_modules/baileys")
    ? "baileys"
...
const connect = async () => {
  ...
  global.client = Socket({
    ...
    browser: ["FreyaBotz", "safari", "1.0.0"],
    auth: state,
  ...
  client.ev.on("messages.upsert", async (chatUpdate) => {
    try {
      ...
      if (!m.message) return;
      Serialize(client, m);
      const files = await Scandir("./src/cmd");
  ...
  client.ev.on("group-participants.update", async (room) => {
    ...
    try {
      pic = await Func.fetchBuffer(
        await client.profilePictureUrl(member, "image")
      );
    } catch {
      pic = await Func.fetchBuffer(
        await client.profilePictureUrl(room.id, "image")
      );
    }
    if (room.action == "add") {
      if (groupSet && groupSet.localonly) {
        if (
          (global.db.users.some((v) => v.jid == member) &&
            !global.db.users.find((v) => v.jid == member).whitelist &&
            !member.startsWith("62")) ||
          !member.startsWith("62")
        ) {
          client.reply(
            room.id,
            Func.texted(
              ...
              `Sorry @${
                member.split`@`[0]
              }, this group is only for indonesian people and you will removed automatically.`
            )
          );
      ...
      let txt = (
        groupSet.text_welcome != "" ? groupSet.text_welcome : text_welcome
      )
        .replace("+tag", `@${member.split`@`[0]}`)
        .replace("+grup", `${meta.subject}`);
      if (groupSet.welcome)
        client.sendMessageModify(room.id, txt, null, {
          largeThumb: true,
          thumbnail: pic,
          url: global.db.setting.link,
        });
    } else if (room.action == "remove") {
      let txt = (groupSet.text_left != "" ? groupSet.text_left : text_left)
        .replace("+tag", `@${member.split`@`[0]}`)
        .replace("+grup", `${meta.subject}`);
      if (groupSet.left)
        client.sendMessageModify(room.id, txt, null, {
