...
module.exports = async (client, m, plugins, store) => {
  try {
    require("./event.database")(m);
    const isOwner = [
      client.decodeJid(client.user.id).split`@`[0],
      global.owner,
      ...
      .includes(m.sender);
    const isPrem =
      (global.db.users.some((v) => v.jid == m.sender) &&
        global.db.users.find((v) => v.jid == m.sender).premium) ||
      isOwner;
    ...
    const isBotAdmin = m.isGroup
      ? adminList.includes(client.user.id.split`:`[0] + "@s.whatsapp.net")
      : false;
    const blockList =
      typeof (await await client.fetchBlocklist()) != "undefined"
        ? await await client.fetchBlocklist()
        : [];
    ...
    if (m.isGroup && !m.isBot && users && users.afk > -1) {
      client.reply(
        m.chat,
        `You are back online after being offline for : ${Func.texted(
          "bold",
          Func.toTime(new Date() - users.afk)
        )}\n\n• ${Func.texted("bold", "Reason")}: ${
          users.afkReason ? users.afkReason : "-"
        }`,
        m
      );
      users.afk = -1;
    ...
    cron.schedule(
      ...
      () => {
        ...
        global.db.users
          .filter((v) => v.limit < global.limit && !v.premium)
          .map((v) => (v.limit = global.limit));
        Object.entries(global.db.statistic).map(
          ([_, prop]) => (prop.today = 0)
        );
      },
      {
    ...
    if (!m.fromMe && m.isBot && m.mtype == "audioMessage" && m.msg.ptt)
    ...
    let isPrefix = (
      setting.multiprefix
        ? setting.prefix.includes(getPrefix)
        : setting.onlyprefix == getPrefix
    )
      ? getPrefix
      : undefined;
    ...
    if (((m.isGroup && !groupSet.mute) || !m.isGroup) && !users.banned) {
      if (body && body == isPrefix) {
        ...
        if (!banchat) {
          ...
          return client.reply(
            m.chat,
            Func.texted("bold", `Response Speed: ${(new Date() - old) * 1}ms`),
            m
          );
        } else {
          await client.reply(m.chat, Func.texted("bold", `Checking . . .`), m);
          return client.reply(
            ...
            Func.texted(
              "bold",
              `Response Speed: ${(new Date() - old) * 1}ms (nonaktif)`
            ),
            m
          );
        }
    ...
    const commands = Func.arrayJoin(
      Object.values(
        Object.fromEntries(
          Object.entries(plugins).filter(([name, prop]) => prop.def.usage)
        )
      ).map((v) => v.def.usage)
    ).concat(
      Func.arrayJoin(
        Object.values(
          Object.fromEntries(
            Object.entries(plugins).filter(([name, prop]) => prop.def.hidden)
          )
        ).map((v) => v.def.hidden)
      )
    );
    ...
    if (
      (body && isPrefix && commands.includes(command)) ||
      (body && !isPrefix && commands.includes(command) && setting.noprefix) ||
      (body &&
        !isPrefix &&
        commands.includes(command) &&
        global.evaluate_chars.includes(command))
    ) {
      const is_commands = Object.fromEntries(
        Object.entries(plugins).filter(([name, prop]) => prop.def.usage)
      );
      let matcher = Func.matcher(command, commands).filter(
        (v) => v.accuracy >= 60
      );
      try {
      ...
      if (!commands.includes(command) && matcher.length > 0 && !setting.self) {
        if (!m.isGroup || (m.isGroup && !groupSet.mute))
          return client.reply(
            m.chat,
            `🚩 Command you are using is wrong, try the following recommendations :\n\n${matcher
              .map(
                (v) =>
                  "➠ *" +
                  (isPrefix ? isPrefix : "") +
      ...
      if (setting.error.includes(command) && !setting.self)
        return client.reply(
          m.chat,
          Func.texted(
            "bold",
            `🚩 Command _${(isPrefix ? isPrefix : "") + command}_ disabled.`
          ),
          m
        );
      ...
      for (let name in is_commands) {
        let cmd = is_commands[name].def;
        let turn =
          cmd.usage instanceof Array
            ? cmd.usage.includes(command)
            : cmd.usage instanceof String
            ? cmd.usage == command
            : false;
        let turn_hidden =
          cmd.hidden instanceof Array
            ? cmd.hidden.includes(command)
            : cmd.hidden instanceof String
            ? cmd.hidden == command
            : false;
        if (
          body &&
          global.evaluate_chars.some((v) => body.startsWith(v)) &&
          !body.startsWith(isPrefix)
        )
          return;
        if (!turn && !turn_hidden) continue;
        if (!m.isGroup && global.blocks.some((no) => m.sender.startsWith(no)))
        ...
        if (setting.pluginDisable.includes(name))
          return client.reply(
            m.chat,
            Func.texted("bold", `🚩 Plugin disabled by Owner.`),
            m
          );
        if (
          !m.isGroup &&
          ...
          !isPrem &&
          !users.banned &&
          new Date() * 1 - chats.lastchat < global.timer
        )
          continue;
        if (
          !m.isGroup &&
          ...
          return client
            .sendMessageModify(
              m.chat,
              `🚩 Using bot in private chat only for premium user, upgrade to premium plan only Rp. 20,000,- to get 1K limits for 1 month.\n\nIf you want to buy contact *${prefixes[0]}owner*`,
              m,
              {
                largeThumb: true,
                thumbnail: await Func.fetchBuffer(
                  "https://telegra.ph/file/0b32e0a0bb3b81fef9838.jpg"
                ),
                url: setting.link,
              }
            )
        ...
        if (
          !["me", "owner"].includes(name) &&
          users &&
          (users.banned || new Date() - users.banTemp < global.timer)
        )
          return;
        if (
          m.isGroup &&
          !["activation", "groupinfo"].includes(name) &&
          groupSet.mute
        )
          continue;
        if (m.isGroup && !isOwner && /chat.whatsapp.com/i.test(text))
        ...
        if (
          ...
          text &&
          new RegExp(
            "\\b" + global.db.setting.toxic.join("\\b|\\b") + "\\b"
          ).test(text.toLowerCase())
        ) {
          client
            .reply(
              m.chat,
              `🚩 You violated the *Terms & Conditions* of using bots by using blacklisted keywords, as a penalty for your violation being blocked and banned. To unblock and unbanned you have to pay *Rp. 10,000,-*`,
              m
            )
            .then(() => {
              users.banned = true;
        ...
        if (cmd.limit && users.limit < 1) {
          return client
            .reply(
              m.chat,
              `🚩 Your bot usage has reached the limit and will be will be reset after 12 hours.\n\nTo get more limits, upgrade to a premium plan send *${prefixes[0]}premium*`,
              m
            )
      ...
      const is_events = Object.fromEntries(
        Object.entries(plugins).filter(([name, prop]) => !prop.def.usage)
      );
      for (let name in is_events) {
        let event = is_events[name].def;
        ...
        if (
          ...
          !isPrem &&
          !users.banned &&
          !["chatAI"].includes(name) &&
          setting.groupmode
        )
          return client
            .sendMessageModify(
              m.chat,
              `🚩 Using bot in private chat only for premium user, upgrade to premium plan only Rp. 10,000,- to get 1K limits for 1 month.\n\nIf you want to buy contact *${prefixes[0]}owner*`,
              m,
              {
                largeThumb: true,
                thumbnail: await Func.fetchBuffer(
        ...
        if (
          ...
          continue;
        if (!["exec", "restrict"].includes(name) && users && users.banned)
          continue;
        if (
          ![
            "anti_link",
        ...
        if (
          ...
          (!setting.autodownload ||
            (body && global.evaluate_chars.some((v) => body.startsWith(v))))
        )
          continue;
        if (event.premium && !isPrem && body && Func.socmed(body))
          return client.reply(m.chat, global.status.premium, m);
        event.async(m, {
          client,
          body,
          participants,
